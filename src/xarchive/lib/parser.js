 ...
 * @returns {{user_id, screen_name, name, description, location, url, profile_url,
 *   profile_image_url, profile_banner_url, verified, followers_count, following_count}}
 */
function extractAuthor(userResult) {
  const userCore = userResult?.core || {};
  const userLegacy = userResult?.legacy || {};
  const screenName = userCore.screen_name || userLegacy.screen_name || null;
  const websiteUrl = userLegacy.entities?.url?.urls?.[0]?.expanded_url || userLegacy.url || null;

  return {
    user_id: userResult?.rest_id || userLegacy.id_str || null,
    // X moved identity fields out of `legacy` and into `core` in newer
    // GraphQL user results. Keep the legacy fallbacks for older responses.
    screen_name: screenName,
    name: userCore.name || userLegacy.name || null,
    description: userLegacy.description || null,
    location: userLegacy.location || null,
    url: websiteUrl,
    profile_url: screenName ? `https://x.com/${screenName}` : null,
    profile_image_url: userResult?.avatar?.image_url || userLegacy.profile_image_url_https || null,
    profile_banner_url: userLegacy.profile_banner_url || null,
    verified: Boolean(
...
function extractQuotedTweet(result) {
  ...
  const userResult = q.core?.user_results?.result;
  const noteText = q.note_tweet?.note_tweet_results?.result?.text;
  const author = extractAuthor(userResult);
